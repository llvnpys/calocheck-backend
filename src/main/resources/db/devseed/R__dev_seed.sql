...
-- =========================================
-- 홍콩반점 메뉴/영양 예시 (price nutrition으로 이동)
-- =========================================
WITH bid AS (
    SELECT id AS brand_id FROM brand WHERE name='홍콩반점'
),
     m1 AS (
INSERT INTO menu (brand_id, name)
SELECT brand_id, '짜장면' FROM bid
    ON CONFLICT (brand_id, name) DO NOTHING
RETURNING id, name
)
SELECT 1;
...
WITH bid AS (
    SELECT id AS brand_id FROM brand WHERE name='홍콩반점'
     ...
     m2 AS (
INSERT INTO menu (brand_id, name)
SELECT brand_id, '짬뽕' FROM bid
    ON CONFLICT (brand_id, name) DO NOTHING
RETURNING id, name
)
SELECT 1;

INSERT INTO nutrition (menu_id, size, calories, protein, fat, carbohydrate, sugar, caffeine_mg, price)
SELECT (SELECT id FROM menu WHERE name='짜장면' AND brand_id=(SELECT id FROM brand WHERE name='홍콩반점')),
       'Regular', 750, 18, 20, 110, 10, NULL, 6000
...
SELECT (SELECT id FROM menu WHERE name='짬뽕' AND brand_id=(SELECT id FROM brand WHERE name='홍콩반점')),
       'Regular', 800, 22, 17, 105, 8, NULL, 7000
    ON CONFLICT (menu_id, size) DO NOTHING;

-- =========================================
-- 매머드커피 메뉴/영양 예시 (price nutrition으로 이동)
-- =========================================
WITH bid AS (
...
WITH bid AS (SELECT id AS brand_id FROM brand WHERE name='매머드커피'),
     m2 AS (
INSERT INTO menu (brand_id, name)
SELECT brand_id, '디카페인 아메리카노' FROM bid
    ON CONFLICT (brand_id, name) DO NOTHING
RETURNING id, name
)
SELECT 1;
